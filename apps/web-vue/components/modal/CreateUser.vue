...
const schema = Yup.object().shape({
  ...
  profile_image_file: Yup.mixed<File>().required(t("field_errors.required_default"))
})
...
function onFilePicked (files: FileList) {
  state.profile_image_file = files[0]
}

async function submit(event: FormSubmitEvent<any>) {
  const file_addr = await s3.upload(state.profile_image_file)
