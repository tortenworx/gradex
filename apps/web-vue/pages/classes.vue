...
<template>
    ...
    <main class="mt-4 grid md:grid-cols-4">
        <div v-for="item in classes" class="bg-white dark:bg-zinc-950 px-3 py-2 border-t-2 border-oct-othagreen rounded-md hover:pointer" @click="openInfo(item)">
            <div>
                <p class="text-zinc-300">{{ item.code }}</p>
                <h1 class="text-xl text-oct-green dark:text-lime-500 font-serif mb-1">{{ item.name }}</h1>
                <p class="font-mono text-zinc-500 tracking-tighter">{{ item.teacher.first_name }} {{ item.teacher.last_name }}</p>
            </div>
        </div>
    </main>
</template>